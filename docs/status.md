...
| ADR-004 | Runtime terrain art presentation: real DEC-007 tiles vs placeholder (interior world anchor, loader, whole-presentation fallback) | `docs/architecture/decisions/ADR-004-runtime-terrain-art.md` |
...
| 2026-09-06 | TOOL-001 / terrain art | asset-processor validated the delivered terrain art with the new `tile` command: all 3 PROCESSED (surface-cap 512x32 earthTopRow 11 / bodyHeight 21; rock 512x64; interior 64x64 seamless H+V; zero seam discontinuities edgeMeanDiff=0), mirrored byte-identical (SHA-256) to `assets/terrain/layers/` + sidecars. Human granted the DEC-006 visual sign-off. Processed assets committed. Next: dev story swaps the placeholder terrain for the real art (loading, earth-top alignment to H(x), interior tiling, band-thickness match with `TerrainConfig`). |
